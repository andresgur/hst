...
errmags = np.array(errmags)[sorting]
...
observed_sp = SourceSpectrum(Empirical1D, points=pivots, lookup_table=fluxes)
...
exponent = 0.1
# fix the amplitude
#powerlaw.set_param_hint("amplitude", value=amplitude, vary=False)
powerlaw.set_param_hint("exponent", max=0, value=-2, min=-10)
params = powerlaw.make_params()
...
try:
    ci = result.conf_interval(sigmas=[1])
    exponent_uncertainty = result.best_values["exponent"] - ci["exponent"][0][1] # 1 sigma uncertainty
except MinimizerException:
    exponent_uncertainty = result.params["exponent"].stderr
exponent_uncertainty = result.params["exponent"].stderr

print("Best-fit powerlaw F $\propto$ $\lambda^{%.3f$\pm$%.3f}" % (result.best_values["exponent"], exponent_uncertainty))
##print(result.best_values["amplitude"])

#powerlaw_uncert = SourceSpectrum(PowerLawFlux1D, alpha=-ci["exponent"][2][1],
#                    amplitude=result.best_values["amplitude"], x_0=1) * deredden

#exponent_uncertainties =  powerlaw_uncert(pivots).value - new_intrinsic_fluxes
#print("Additional uncertainties")
#print(exponent_uncertainties)
#print("Existing uncertainties")
#yerr = np.sqrt(yerr**2 + exponent_uncertainties**2
...
plt.xlabel("$\AA$")
plt.errorbar(pivots, fluxes, yerr=yerr, fmt="o", label="Old observed")
plt.errorbar(pivots, new_intrinsic_fluxes, yerr=yerr, fmt="o", label="New unabsorbed")
plt.legend()
...
old_fluxes = ["%.2f$\pm$%.2f" %(flux/ scale, err/ scale) for flux, err in zip(fluxes, yerr)]
...
new_fluxes_str = ["%.2f$\pm$%.2f" %(flux/ scale, err/ scale) for flux, err in zip(new_intrinsic_fluxes, yerr)]
